@Regression @HeroApp
Feature: HeroApp Functionality

  Background:
      Given user navigates to "http://the-internet.herokuapp.com/"
    #BackGround is a group of all common steps between the feature scenarios.
    #BackGround is only for the beginning steps that are identical - could be the 1st step only, could be the first 5 steps
